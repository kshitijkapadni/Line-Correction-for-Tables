...
def line_detection(image):
    image_gray = cv2.copyMakeBorder(image,10,10,10,10,cv2.BORDER_CONSTANT,value=[255,255,255])
    ...
    horizontal = cv2.dilate(horizontal, (1,1), iterations=8)
    horizontal = cv2.erode(horizontal, (1,1), iterations=7)
    ...
    hor_lines = cv2.HoughLinesP(horizontal,rho=1,theta=np.pi/180,threshold=40,minLineLength=10,maxLineGap=20)
    ...
    #     cv2.line(img, (x1,y1-5), (x2,y2-5), (0, 255, 0), 2)
    ...
    verticalStructure = cv2.getStructuringElement(cv2.MORPH_RECT, (1,7))

    # Apply morphology operations
    vertical = cv2.erode(vertical, verticalStructure)
    vertical = cv2.dilate(vertical, verticalStructure)

    vertical = cv2.dilate(vertical, (1,1), iterations=3)
    vertical = cv2.erode(vertical, (1,5), iterations=15)
    ...
    ver_lines = cv2.HoughLinesP(vertical, 1, np.pi/180, 40, np.array([]), 80, 10)
    ...
    for x1, y1, x2, y2 in ver:
        cv2.line(post, (x1+1,y1-8), (x2+1,y2), (0, 0, 0), 1)

    for x1, y1, x2, y2 in hor:
        cv2.line(post, (x1,y1-1), (x2,y2-1), (0, 0, 0), 1)
    
    cv2.imshow("image",post)
    cv2.imwrite("fix3.jpg",post)
    cv2.waitKey(0)
    #######################################################################

    return hor,ver

image = cv2.imread('Broken\\broken3.PNG')
print(image.shape)
detection = line_detection(image)